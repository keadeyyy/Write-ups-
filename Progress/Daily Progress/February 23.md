# 📊 Individual Progress Scorecard

## Daily Training Log

**Date:** February 23, 2026 | **Training Day:** 9/90

### 1. Time Investment

- [✔] Training time today: 1 hour
- [✔] Goal met? (Yes/No)
- [✔] Consistency streak: 9 days

**Points:**

- 2+ hours = 10 points
- 1-2 hours = 5 points
- <1 hour = 2 points
- Missed day = 0 points (streak resets)

### 2. Challenge Completion

| Difficulty      | Challenges Solved | Points Earned |
| --------------- | ----------------- | ------------- |
| Easy            | 1 × 5 pts     | 5         |
| Medium          | _____ × 15 pts    | _____         |
| Hard            | _____ × 30 pts    | _____         |
| Expert          | _____ × 50 pts    | _____         |
| **Daily Total** |                   | **_____**     |

### 3. Quality Indicators

- [✔] Created writeup for at least 1 challenge (+10 pts)
- [✔] Reviewed 3+ writeups from others (+5 pts)
- [✔] Learned new technique/tool (+10 pts)
- [ ] Updated cheat sheet (+5 pts)
- [ ] Practiced timed challenge (+5 pts)

**Quality Points Total:** 25

### 4. Category Focus Today

Which categories did you practice?

- [ ] Web Exploitation
- [ ] Binary Exploitation / Pwn
- [✔] Cryptography
- [ ] Reverse Engineering
- [ ] Forensics
- [ ] OSINT
- [✔] Other: Networking

**Primary Specialty:** 45 minutes

**Secondary Specialty:** 0 minutes

**Other Categories:** 15 minutes

### 5. Reflection (Qualitative)

**What went well today?**

- Successfully connected to a remote challenge server and retrieved the flag.

**What challenged you?**

- Understanding how JSON requests are sent to servers using Python.

**Key learning:**

- Cryptography challenges may involve network communication and interacting with remote services.

**Tomorrow's focus:**

- Continue solving encoding challenges like ASCII and Hex.
